  ...
  async function render(state, mount) {
    ...
    const root = el('div', { class: 'card' },
      ...
      el('div', { style: { fontSize: '11px', color: 'var(--fg-mute)', marginBottom: '6px' } },
        ...
        el('br'),
        '「コピー後削除」: コピー中と書き込み後の二重ハッシュ照合に成功した場合のみ、元データ（src）を削除する。'
        + '削除直前にもファイル変更がないことを確認します。重複スキップ時は同じ患者・種別の既存コピーを確認できた場合のみ削除。'
        + '保存先フォルダ内のデータを取り込み元にした場合は削除しません。空になった取り込み元フォルダは症例フォルダごと自動削除されます。'),
      (function() {
