...
def build_pdf():
    ...
    story.append(Paragraph(
        ...
        style_body))
    story.append(Paragraph(
        '<b>「コピー後削除」列</b>：チェックを入れた種別は、コピー成功後に元データ（SD カード側）を削除します。'
        '安全のため <b>二重ハッシュ照合</b>（コピー中に読んだ内容のハッシュと、NAS に書き込まれた実データを読み戻したハッシュの一致）に成功し、'
        'さらに <b>削除直前にファイルが変更されていないこと</b>（サイズ・更新時刻の照合）を確認できた場合のみ削除します。',
        style_body))
    story.append(Paragraph(
        '過去に取り込み済みの同一ファイル（重複スキップ扱い）も、<b>同じ患者・同じ種別</b>として取り込み済みで、'
        'かつ NAS 上の既存コピーの実在と内容一致を確認できた場合のみ元データを削除します。'
        '別患者・別種別として保存されている同一内容のファイルを根拠に削除することはありません。',
        style_body))
    story.append(Paragraph(
        'すべてのファイルを移し終えて空になった取り込み元のフォルダ（症例フォルダ）は、自動的に削除されます'
        '（.DS_Store 等の macOS メタファイルのみが残ったフォルダも「空」として扱います。'
        '削除が実際に行われた場所の親フォルダだけが対象で、取り込みと無関係な空フォルダには触れません。'
        'SD カードのマウント直下や取り込み元フォルダの外は絶対に削除しません）。',
        style_body))
    story.append(Paragraph(
        '⚠ 安全ガード: NAS 等の保存先フォルダの中にあるデータを取り込み元に選んだ場合'
        '（例: 過去の患者フォルダから別患者へ複製する場合）は、「コピー後削除」が ON でも元データは削除されません。'
        '保存領域内のデータをこのアプリが消すことはありません。',
        style_note))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        ...
        'ここで計算したハッシュは <b>コピー時に再利用</b> されるため、無駄な読み直しが発生しません（v0.3.16 の高速化）。',
        style_body))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        '「コピー開始」を押すとファイルが <b>最大 3 並列</b> でコピーされます（v0.3.16 で高速化。'
        'コピーと同時にハッシュを計算する方式に変更し、SD カードの読み取り回数を従来の 1/3 に削減。'
        '写真が大量にある場合や大きな動画で、従来比 2〜3 倍の速度になります）。',
        style_body))
    story.append(Paragraph(
        '<b>各ファイルでコピー後にハッシュリチェック</b>（コピー中に読んだ内容と、書き込まれた dst を読み戻した SHA-256 の比較）を実施。'
        '不一致なら dst を削除して失敗扱いになります。'
        '<b>削除設定の種別</b>の場合、この照合成功に加えて <b>削除直前に src が変更されていないこと</b> を確認してから削除します。'
        '進捗は「完了ファイル数 / 全体」で表示されます。右下の <b>赤い「中断」ボタン</b>で進行中のコピーを止められます。',
        style_body))
    ...
    story.append(Paragraph(
        ...
        '<b>v0.3.19</b>: ネットワークが遅くても応答が続く限り送信を継続します（無応答 60 秒で打ち切り）。'
        '送信に失敗した写真があるとポップアップで知らせ、「失敗分を再送信」でその場で失敗分だけを'
        '同じ Study に送り直せます（送信済み分と重複しません）。あとで送る場合は開始画面の'
        '「未送信の DICOM 写真が N 件あります」バナー→「再送画面を開く」から再送できます。',
        style_body))
    story.append(Paragraph(
        '<b>大量送信時</b>（1000 枚など）は内部で <b>20 枚ずつのバッチ処理</b> に分割され、'
        'メモリ枯渇によるフリーズを防ぎます。同一 StudyInstanceUID で送るので PACS 側では 1 つの Study として認識されます。'
        '失敗時は再送キューに自動登録され、失敗分だけをあとから送り直せます。',
        style_body))
    ...
    story.append(Paragraph(
        ...
        '（v0.3.16 から、大きな動画のハッシュ計算中でも即座に中断できます）。',
        style_body))
    story.append(Paragraph(
        '中断後の状態:',
        style_body))
    story.append(Paragraph(
        '・進行中だったファイル → 中途半端な dst を削除（部分ファイルは残らない）<br/>'
        '・既にコピー完了したファイル → そのまま保持<br/>'
        '・元データ（src）は、コピー＆照合が完了して削除済みの分を除き、すべて残る<br/>'
        '・空フォルダの後片付け（症例フォルダ削除）は中断時には実行されない<br/>'
...
└── 内視鏡/"""
    ...
    trouble = [
        ...
        ['DICOM 送信が失敗する', 'C-ECHO で疎通確認 → AE Title / Host / Port 見直し。失敗分はポップアップの「失敗分を再送信」でその場で、'
         'またはあとで開始画面の「未送信の DICOM 写真が N 件あります」バナー→「再送画面を開く」から再送できる'],
        ...
        ['「コピー後削除」設定なのに残ってる', '照合失敗や削除前検証（ファイル変更の形跡）で安全のため削除中止。コピー画面の警告ログを確認'],
        ['SD カードの症例フォルダが消えた', '仕様です。「コピー後削除」の種別で全ファイルを移し終えて空になったフォルダは自動削除される（v0.3.16）'],
        ['取り込みが遅い', 'v0.3.16 で 2〜3 倍高速化（並列コピー＋読み取り削減）。それでも遅い場合は SD カードリーダーや NAS の接続（有線 LAN 推奨）を確認'],
    ]
    ...
    story.append(Paragraph('10.3 データ完全性チェックの動作（v0.3.16）', style_step))
    story.append(Paragraph(
        '医療データの完全性確保のため、各ファイルで以下のチェックを実施します。'
        'v0.3.16 から「コピーしながら同時にハッシュを計算する」方式となり、'
        '完全性の保証を保ったまま SD カードの読み取り回数を削減しています。', style_body))
    recheck = [
        ['タイミング', '内容'],
        ['プレビュー前（差分インポート ON 時）', '全ファイルの SHA-256 を計算し、取り込み履歴 DB と照合して既取込を検出。このハッシュはコピー時に再利用。'],
        ['コピー中', '読み取ったデータの SHA-256 をコピーと同時に計算（インラインハッシュ）。'],
        ['コピー直後', 'NAS に書き込まれた dst を読み戻して SHA-256 を計算し、コピー中のハッシュと比較。不一致なら dst を削除して失敗扱い。'],
        ['削除直前（「コピー後削除」設定時のみ）',
         '上記の二重照合成功に加えて、src のサイズ・更新時刻がコピー開始時から変化していないこと、'
         'src が保存先フォルダ内でないことを確認。条件を満たさなければ削除中止（警告表示）。'],
        ['重複スキップ時の削除（「コピー後削除」設定時のみ）',
         '既存コピーが「同じ患者・同じ種別」の記録で、取り込み元自身でなく、読み戻したハッシュが一致し、'
         'さらに src が事前チェック以降変更されていないことをすべて確認してから削除。1つでも満たさなければ削除中止（警告表示）。'],
    ]
    story.append(make_table(recheck, col_widths=[60 * mm, 100 * mm]))
    story.append(Paragraph('10.4 v0.3.16 の高速化の仕組み（参考）', style_step))
    speed = [
        ['項目', '内容'],
        ['読み取り回数の削減', '従来: 事前ハッシュ＋コピー＋削除前再ハッシュで src を最大 3 回読み → 現在: コピー時の 1 回のみ'],
        ['事前チェック結果の再利用', 'プレビュー前に計算したハッシュをコピー時にそのまま利用'],
        ['並列コピー', '最大 3 ファイルを同時にコピー（大量の写真で効果大）'],
        ['大容量バッファ', '読み書きのバッファを 64KB → 8MB に拡大（NAS・SD カードで効果大）'],
        ['履歴 DB の書き込みまとめ', 'ファイルごとの逐次書き込みをまとめ書きに変更'],
    ]
    story.append(make_table(speed, col_widths=[55 * mm, 105 * mm]))
